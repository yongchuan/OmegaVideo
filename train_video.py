...
from models.sit import SiT_models as ImageSiTModels
from models.sit_video import SiT_models as VideoSiTModels
from loss_video import SILoss
from utils import load_encoders

from dataset import CustomDataset
from json_label_dataset import JsonLabelDataset
from video_dataset import VideoLatentDataset, pad_video_latent_collate
...
def main(args):    
    ...
    if args.use_video_dataset:
        if not args.use_json_dataset:
            raise ValueError("--use-video-dataset currently requires --use-json-dataset.")
        if not args.label_file:
            raise ValueError("--use-video-dataset requires --label-file with video captions.")
    
    train_dataset = None
    video_latent_shape = None

    if args.use_video_dataset:
        train_dataset = VideoLatentDataset(
            args.data_dir,
    ...
    if args.use_video_dataset:
        _latent_c, _latent_t, latent_h, latent_w = video_latent_shape
        model = VideoSiTModels[args.model](
        ...
        if head_dim % 3 != 0:
            raise ValueError(
                f"Video model {args.model} has attention head dim {head_dim}, which is not divisible by 3. "
                "Use a video model variant whose head dim is divisible by 3, such as SiT-XL/*."
            )
    else:
        z_dims = [encoder.embed_dim for encoder in encoders] if args.enc_type != 'None' else [0]
        model = ImageSiTModels[args.model](
    ...
    # Setup data:
    if train_dataset is None and args.use_json_dataset:
    ...
    train_dataloader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
        collate_fn=pad_video_latent_collate if args.use_video_dataset else None,
    )
    if accelerator.is_main_process:
        dataset_kind = "videos" if args.use_video_dataset else "images"
        logger.info(f"Dataset contains {len(train_dataset):,} {dataset_kind} ({args.data_dir})")
    ...
    for epoch in range(args.epochs):
        ...
        for raw_image, x, y in train_dataloader:
            ...
            running_loss += accelerator.gather(loss_mean).mean().detach().item()
            log_steps += 1

            if accelerator.sync_gradients and global_step % 100 == 0 and global_step > 0:
